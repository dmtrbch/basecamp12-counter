...
const Home = () => {
    ...
    calls: [
      {
        ...
        args: [counter?.address, contractBalance ? BigInt(Number(contractBalance)) : 0n],
      },
      {
        contractName: "Counter",
  ...
  const {data: events} = useScaffoldEventHistory({
    ...
    fromBlock: blockNumber ? (blockNumber > 50 ? BigInt(blockNumber - 50) : 0n) : 0n,
    watch: true,
  });
              ...
              {events && events.length > 0 ? (
                events.map((event, index) => (
                  <div key={index} className="p-4 bg-base-200 rounded-xl">
                    <p className="text-lg">
                      <span className="font-medium">{event.parsedArgs.account.substring(0, 6)}...{event.parsedArgs.account.substring(event.parsedArgs.account.length - 4)} incremented the counter</span>
                    </p>
                  </div>
                ))
              ) : (
